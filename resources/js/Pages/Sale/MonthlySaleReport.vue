<template>
    ...
    <div class="animate__animated animate__fadeIn">
            ...
            <div class="col-12 mb-3">
                <div class="border p-3 shadow rounded">
                    <nav aria-label="breadcrumb">
                        <ol
                            class="breadcrumb mb-0 d-flex justify-content-between"
                        >
                            ...
                            <div class="input-group align-items-stretch w-auto">
                                <date-picker
                                    ...
                                    type="month"
                                    :clearable="false"
                                    :default-value="form.month"
                                    :disabled-month="disabledMonths"
                                    @change="monthHandler"
                                ></date-picker>
                                <button
        ...
        <div
            v-if="thisMonthSaleMonth.length == 0"
                    ...
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            <div>
                                <line-chart
                                    :label="'Months/MMk'"
                                    :labels="thisMonthSaleMonth"
                                    :data="thisMonthSaleTotal"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body pb-0 table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                <tr
                                    v-for="vl in voucherLists.data"
                                    :key="vl.id"
                                >
                                    <td>
                                        {{ vl.voucherResource.customer_name }}
                                    </td>
                                    <td class="text-truncate text-nowrap">
                                        {{ vl.item_name }}
                                    </td>
                                    <td>
                                        {{ vl.voucherResource.voucher_number }}
                                    </td>
                                    <td>{{ vl.unit_price }}</td>
                                    <td>{{ vl.quantity }}</td>
                                    <td>{{ vl.cost }} MMK</td>
...
export default {
    ...
    setup(props) {
        ...
        // left analy
        let total = computed(() =>
            props.thisMonthSaleTotal.reduce((pv, c) => pv + Number(c), 0)
        );
        ...
        const monthHandler = () => {
            if (!form.month) {
                showAlert("error", "Please choose month", "");
                return;
            }
            form.get(route("monthlySaleReport"), {
                onSuccess: () => {
